    ...
    <div style={{ minHeight: "100vh", background: t.bg, fontFamily: "'Syne', sans-serif", color: t.text, overflowX: "hidden", position: "relative", transition: "background 0.4s, color 0.4s" }}>
      ...
      <footer style={{ position: "relative", zIndex: 1, background: t.footerBg, color: "white", padding: "56px 80px 36px", transition: "background 0.4s" }}>
        ...
        <div style={{ borderTop: `1px solid ${dark ? "#1a1740" : "#1e1b2e"}`, paddingTop: 22, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: 11, color: dark ? "#2a2750" : "#333", fontFamily: "'DM Sans', sans-serif" }}>© 2025 InstantBI. Powered by Gemini AI.</span>
...
// ─────────────────────────────────────────────
// TOOL PAGE  (original Gemini logic, new design)
// ─────────────────────────────────────────────
function ToolPage({ onBack, dark, onToggleDark }) {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setCsvData(data);
        setHeaders(hdrs);
        setMessages([{ role: "system", content: `Dataset loaded: ${data.length} rows • ${hdrs.length} columns` }]);
        setDashboard(null);
        setError(null);
      } catch {
  ...
  const handleQuery = async (e) => {
    ...
    const sampleData = JSON.stringify(csvData.slice(0, 5));
    const prompt = `
...
`;
    try {
      ...
      setDashboard(dash);
      setMessages(prev => [...prev, { role: "assistant", content: dash.summary }]);
    } catch (err) {
  ...
  // ── Chart renderer (styled like reference) ──
  const renderChart = (chart, index) => {
    const isPie = chart.type === "pie";
    const ChartComponent = { bar: BarChart, line: LineChart, pie: PieChart, area: AreaChart }[chart.type] || BarChart;

    // Auto-aggregate repeated x values
    const aggregated = (() => {
      const map = {};
      csvData.forEach(row => {
        const xVal = row[chart.xAxisKey];
        const yVal = parseFloat(row[chart.yAxisKey]) || 0;
        if (xVal === undefined || xVal === null || xVal === "") return;
        if (map[xVal] === undefined) map[xVal] = 0;
        map[xVal] += yVal;
      });
      return Object.entries(map).map(([key, val]) => ({
        [chart.xAxisKey]: key,
        [chart.yAxisKey]: Math.round(val * 100) / 100,
      }));
    })();

    const xVals = csvData.map(r => r[chart.xAxisKey]);
    const hasRepeats = new Set(xVals).size < xVals.length;
    const chartData = hasRepeats ? aggregated : csvData;
      ...
      <div key={index} style={{ background: t.chartCard, borderRadius: 18, padding: 24, border: `1px solid ${t.chartCardBorder}`, boxShadow: "0 4px 20px rgba(0,0,0,0.06)", transition: "background 0.4s, border-color 0.4s" }}>
        ...
        <ResponsiveContainer width="100%" height={240}>
          <ChartComponent data={chartData} margin={{ top: 8, right: 24, left: 0, bottom: 16 }}>
            ...
            {chart.type === "pie" && (
              <Pie data={chartData} dataKey={chart.yAxisKey} nameKey={chart.xAxisKey} outerRadius={95} innerRadius={55} paddingAngle={4} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
            ...
            {chart.type === "bar" && (
              <Bar dataKey={chart.yAxisKey} radius={[5, 5, 0, 0]} barSize={chartData.length > 10 ? 16 : 28}>
                {chartData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Bar>
            )}
          </ChartComponent>
        ...
        {!dashboard ? (
          ...
          <div style={{ padding: "32px 40px 72px", maxWidth: 1160, margin: "0 auto" }}>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 18 }}>
              ...
              <button
                onClick={() => {
                  if (!dashboard || !csvData) return;
                  const hdrs = Object.keys(csvData[0]).join(",");
                  const rows = csvData.map(r => Object.values(r).join(",")).join("\n");
                  const blob = new Blob([hdrs + "\n" + rows], { type: "text/csv" });
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement("a");
                  a.href = url;
                  a.download = `${dashboard.title.replace(/\s+/g, "_")}_export.csv`;
                  a.click();
